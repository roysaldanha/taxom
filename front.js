...
function woof_checkboxes_slide() {
	...
	if (childs.size()) {
	    ...
	    for (var i = 0, n = hiddenLis.length; i < n; i++) { 
	    	$(".woof_li_term_"+hiddenLis[i]).hide();
	    }


for (var i = 0, n = shownLis.length; i < n; i++) { 
	    	$(".woof_li_term_"+shownLis[i]).show();
	    }
		...
		jQuery(a).click(function () {
			...
			var mmmm = jQuery(this).parent();
			mmmm.find('li').show(333);
...
$(document).ready(function(){
	...
	$(".expand_button").click(function(){
		...
		if ($(this).hasClass('poof_is_closed')) {
		//lets open
		jQuery(this).parent().find('ul').show();
		$(this).removeClass('poof_is_closed');
		$(this).addClass('poof_is_opened');
		$(this).text('-');
	    } else {
		var tesdf = jQuery(this).parent(); console.log(tesdf);
		jQuery(this).parent().find('ul').hide();
		$(this).removeClass('poof_is_opened');
		$(this).addClass('poof_is_closed');
		$(this).text('+');
	    }
