...
//***
	...
	public function woof_draw_checkbox_childs_own($taxonomy_info, $tax_slug, $term_id, $childs, $show_count, $show_count_dynamic, $hide_dynamic_empty_pos, $new_cats) {
		    
    	$childs = apply_filters('woof_sort_terms_before_out', $childs, 'checkbox');

		$shop_page_url = get_permalink( woocommerce_get_page_id( 'shop' ) ); 
    	if (!empty($childs) AND is_array($childs)) {
    		foreach ($childs as $term) {  $inique_id = uniqid();
                $count_string = "";
                $count = 0;
                ...
                if (in_array($term['term_id'], $hidden_terms))
                {
        	 	...
        	 	if(in_array($term['term_id'], $new_cats)) {  ?> 
        	 		<ul class="expand_button_<?php echo $term_id; ?>">
        	 			<li class="container">
						...
						<?php } ?>
						 <a href="<?php echo $shop_page_url; ?>?swoof=1&product_cat=<?php echo $term['slug']; ?>"><?php echo $term['name']; ?></a>
						 	<?php if (!empty($term['childs']))
	                        {
	                            $this->woof_draw_checkbox_childs_own($taxonomy_info, $tax_slug, $term['term_id'], $term['childs'], $show_count, $show_count_dynamic, $hide_dynamic_empty_pos, $new_cats);
	                        } ?>
                    	</li>
                	</ul> 
            	<?php } else {
	...
	public function woof_draw_checkbox_childs_golmaal($taxonomy_info, $tax_slug, $term_id, $childs, $show_count, $show_count_dynamic, $hide_dynamic_empty_pos) {
 ...
 ?> 
        	 		<ul class="expand_button_<?php echo $term_id; ?>">
        	 			<li class="container" style="border-left: 0.1em dotted #000000;background: url('http://www.mothsofindia.org/graphics/dotted.gif') no-repeat scroll 1px 0.9em transparent;">
						<?php if(!empty($term['childs'])) { ?>
							<a href="javascript: void(0);" class="expand_button poof_is_opened" id="expand_button_<?php echo $term['term_id']; ?>">-</a>
						<?php } ?>
						 <a href="<?php echo $shop_page_url; ?>?swoof=1&product_cat=<?php echo $term['slug']; ?>"><?php echo $term['name']; ?></a>
						 	<?php if (!empty($term['childs']))
	                        {
	                            $this->woof_draw_checkbox_childs_golmaal($taxonomy_info, $tax_slug, $term['term_id'], $term['childs'], $show_count, $show_count_dynamic, $hide_dynamic_empty_pos);
	                        } ?>
                    	</li>
                	</ul> 
            	<?php
