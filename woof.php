...
if (!function_exists('woof_print_tax')) {
	...
	switch ($woof_settings['tax_type'][$tax_slug]) {
		...
		if ($WOOF->settings['show_title_label'][$tax_slug]) {
			...
			<?php if ( is_product() && $tax_slug == 'product_cat') { ?>
			<?php echo $WOOF->render_html(WOOF_PATH . 'views/html_types/boom.php', $args);
		} else  if ($tax_slug == 'product_cat') { 
 			echo $WOOF->render_html(WOOF_PATH . 'views/html_types/taxonomy.php', $args);
		} else {
